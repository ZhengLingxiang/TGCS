...
class SaliencyGate(nn.Module):
    """Soft gate per token in [0,1]; keep cls token unchanged."""
...
class MultiScaleAggregator(nn.Module):
    """Aggregate neighboring patch tokens at two kNN scales on group centers.
       Returns residual to add to patch tokens (exclude cls)."""
...
@MODELS.register_module()
class GPT_Transformer_TGCS(nn.Module):
    def __init__(self, config, **kwargs):
        super().__init__()
        self.config = config
        ...
        self.show_attn = getattr(config, 'show_attn', {}) if hasattr(config, 'show_attn') else {}
    ...
    def forward(self, pts):
        ...
        attn_mask = torch.full((L + 2, L + 2), -float("Inf"), device=group_input_tokens.device, dtype=group_input_tokens.dtype).to(torch.bool)

        attn_mask = torch.triu(attn_mask, diagonal=1)

        # transformer
        ret, concat_f = self.blocks(x, pos, attn_mask, classify=True, xScale_pos3d=center)

        # encoded_features = torch.cat([encoded_features[:, 0, :].unsqueeze(1), encoded_features[:, 2:-1, :]], dim=1)
        # attn_mask = torch.full((L, L), -float("Inf"), device=group_input_tokens.device, dtype=group_input_tokens.dtype).to(torch.bool)
        # attn_mask = torch.triu(attn_mask, diagonal=1)
