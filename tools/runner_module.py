...
import torch.nn as nn
def freeze_bn(model: nn.Module, freeze_affine: bool = True):
...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    for name, param in base_model.named_parameters():
        if ('cls_pos' in name or 'cls_token' in name or 'cls_norm' in name or 'cls_head' in name or
                ('gate' in name or 'ms_agg' in name or 'adapter' in name)):
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        builder.save_checkpoint(base_model, optimizer, epoch, metrics, best_metrics, 'ckpt-last', args, logger = logger) # +str(epoch)+str(metrics.acc)
    if train_writer is not None:
        train_writer.close()
